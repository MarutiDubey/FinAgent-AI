...
const getContainingBlock = element => {
  ...
  while (node && node !== document.documentElement) {
    ...
    if (
      ...
      style.willChange.includes('transform') || style.willChange.includes('perspective') ||
      style.willChange.includes('filter') || /paint|layout|strict|content/.test(style.contain)
    ) return node;
...
const TargetCursor = ({
  ...
  useEffect(() => {
    ...
    const scrollHandler = () => {
      ...
      const isStillOver = elementUnderMouse && (elementUnderMouse === activeTarget || elementUnderMouse.closest(targetSelector) === activeTarget);
    ...
    const enterHandler = e => {
      ...
      const leaveHandler = () => {
        ...
        if (cornersRef.current) {
          ...
          const positions = [{ x: -cs2 * 1.5, y: -cs2 * 1.5 }, { x: cs2 * 0.5, y: -cs2 * 1.5 }, { x: cs2 * 0.5, y: cs2 * 0.5 }, { x: -cs2 * 1.5, y: cs2 * 0.5 }];
